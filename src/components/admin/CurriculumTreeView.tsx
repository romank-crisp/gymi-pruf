 ...
 *
 * Loads all Domains / Modules / Units / Concepts in parallel with
...
export default async function CurriculumTreeView({
  ...
  const [domains, modules, units, concepts] = await Promise.all([
  ...
  const unitsByModule = new Map<Id, typeof units.docs>()
  for (const u of units.docs) {
    const parent = relId(u.module as RelId)
    if (!parent) continue
    const bucket = unitsByModule.get(parent) ?? []
    bucket.push(u)
    unitsByModule.set(parent, bucket)
  }
      ...
      <Gutter className="curriculum-tree">
        <header className="curriculum-tree__header">
          ...
          <p className="curriculum-tree__subtitle">
            Domain → Module → Unit → Concept. Click any node to open its
            detail page. {' '}
                  ...
                  <details open>
                    ...
                    {mods.length === 0 ? (
                      ...
                      <ul className="curriculum-tree__list">
                        {mods.map((mod) => {
                          const uns = unitsByModule.get(mod.id) ?? []
                          return (
